...
class DatasetMedical(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(batch):
            try:
                img_mask = (
                    np.array(Image.open(os.path.join(self.root, mask[i])).convert("L"))
                    > 0
                )
                img_mask = Image.fromarray(img_mask.astype(np.uint8) * 255, mode="L")
            except:
                img_mask = Image.fromarray(
                    np.zeros((self.img_size, self.img_size)), mode="L"
                )
        ...
        for cls_name in self.cls_names:
            ...
            if CLASS_INDEX[cls_name] < 0:
                for image_index in range(
                    0, len(self.meta_info[cls_name]), self.batch_size
                ):
                    file_path = []
                    img_label = []
                    for batch_count in range(0, self.batch_size):
                        if image_index + batch_count >= len(self.meta_info[cls_name]):
                            break
                        file_path.append(
                            self.meta_info[cls_name][image_index + batch_count][
                                "img_path"
                            ]
                        )
                        img_label.append(
                ...
                for image_index in range(
                    ...
                    for batch_count in range(0, self.batch_size):
                        if image_index + batch_count >= len(self.meta_info[cls_name]):
                            break
                        single_file_path = self.meta_info[cls_name][
                            image_index + batch_count
                        ]["img_path"]
                        single_label = self.meta_info[cls_name][
                            image_index + batch_count
                        ]["anomaly"]
                        ...
                        if single_label == 0:
                            ...
                            gt_path.append(
                                single_file_path.replace("img", "anomaly_mask")
                            )
