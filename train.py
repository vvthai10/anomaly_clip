...
def train(args):
    ...
    ##########################################################################################
    clip_model = create_model(model_name='ViT-L-14-336', img_size=args.image_size, device=device,
                              pretrained='openai', require_pretrained=True)
    clip_model.eval()

    vision_learner = AnomalyCLIP_VisionLearner(clip_model)
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        for items in tqdm(train_dataloader):
            ...
            with torch.no_grad():
                ...
                image_features, seg_patch_features = vision_learner.encode_image_learn(image=image)
                image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                seg_patch_features = [p[:, 1:, :] for p in seg_patch_features]
                # det_patch_features = [p[:, 1:, :] for p in det_patch_features]
            ...
            for layer in range(len(seg_patch_features)):
                seg_patch_features[layer] = seg_patch_features[layer] / seg_patch_features[layer].norm(dim=-1,
                                                                                                 keepdim=True)
                anomaly_map = (100.0 * seg_patch_features[layer] @ text_features[0].t())
        ...
        if (epoch + 1) % args.print_freq == 0:
            logger.info(
                'epoch [{}/{}], total_loss:{:.4f}, loss:{:.4f}, image_loss:{:.4f}'.format(epoch + 1, args.epoch, np.mean(total_loss_list), np.mean(loss_list),
