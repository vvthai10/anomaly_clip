    ...
    for epoch in tqdm(range(args.epoch), position=0, leave=True):
        ...
        for items in tqdm(train_dataloader, position=0, leave=True):
            ...
            for idx, patch_feature in enumerate(det_patch_features):
                if idx >= args.feature_map_layer[0]:
                    ...
                    similarity_list.append(det_score)

            label = label.to(device)
            for i in range(len(similarity_list)):
                loss += loss_bce(similarity_list[i], label.float())
            ...
            for idx, patch_feature in enumerate(seg_patch_features):
                if idx >= args.feature_map_layer[0]:
                    patch_feature = patch_feature / patch_feature.norm(dim=-1, keepdim=True)
                    similarity, _ = AnomalyCLIP_lib.compute_similarity(patch_feature, text_features[0])
                    similarity_map = AnomalyCLIP_lib.get_similarity_map(similarity[:, 1:, :],
        ...
        if (epoch + 1) % args.save_freq == 0:
            ...
            torch.save({"prompt_learner": prompt_learner.state_dict(),
                        'seg_adapters': vision_learner.seg_adapters.state_dict(),
                        'det_adapters': vision_learner.det_adapters.state_dict()}, ckp_path)
